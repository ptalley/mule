...
public class ParameterModelValidatorTestCase extends AbstractMuleTestCase {
  ...
  public void before() {
    ...
    ExtensionsTestUtils.mockSubTypes(extensionModel, new HashMap<>());
    ...
    when(validParameterModel.getDslModel()).thenReturn(new ImmutableElementDslModel());
    when(invalidParameterModel.getModelProperty(ParameterGroupModelProperty.class)).thenReturn(Optional.empty());
    when(invalidParameterModel.getDslModel()).thenReturn(new ImmutableElementDslModel());
